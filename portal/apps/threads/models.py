...
class AerpawThread(BaseModel, models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_error == True:
            ...
            message = f'Error# {self.id}:<p class="text-danger">An error has occured and {self.get_target_display()} could not be completed! <br>If this error persists, please <a class="btn btn-sm btn-outline-danger" href="mailto:cjr47@cornell.edu?subject=Error#%20{self.id}%20in%20{self.get_target_display()}">click here to email the Aerpaw Ops Team <i class="fa fa-paper-plane"></i></a></p><hr class="w-50 text-danger">'
        elif self.is_error != True and self.thread_end != None:
            message = f'{self.get_target_display()} is successful!'
            self.is_error = False
...
class ThreadQue(AuditModelMixin, BaseModel, models.Model):

    threads = models.ManyToManyField('AerpawThread')
    is_threading = models.BooleanField(default=False)
    target = models.CharField(max_length=255, choices=ThreadTargets.choices)
