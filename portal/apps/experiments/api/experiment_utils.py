...
def active_development_to_wait_emulation_schedule(request, experiment: AerpawExperiment):
    print(f'Active development to wait emulation schedule')

    dev_session = OnDemandSession.objects.filter(
        experiment=experiment, 
                session_type=OnDemandSession.SessionType.DEVELOPMENT, 
                is_active=True
            ).first()
    stop_experiment_session(dev_session, request.user)
    create_experiment_scheduled_session('emulation', experiment, request.user)
    experiment.experiment_state = 'wait_emulation_schedule'
    experiment.save()
...
def wait_emulation_schedule_to_wait_emulation_deploy(request, experiment: AerpawExperiment):
    ...
    # Remove the tap interface from the bridge so that the experimenter loses access
    connect_user = False
    command = f'sudo python3 /home/aerpawops/AERPAW-Dev/DCS/platform_control/utils/ap-cf-ops-exp-user-connection.py {experiment.canonical_number.canonical_number} {connect_user}'
...
def wait_emulation_deploy_to_active_emulation(request, experiment: AerpawExperiment):
    ...
    # ACTION ITEMS:
    print()
    print('Starting active Emulation Session')
    # get active session and start
    session = OnDemandSession.objects.filter(
        experiment_id=experiment.id,
        session_type=OnDemandSession.SessionType.EMULATION,
    ...
    experiment.save()
    print(f'Setting Experiment state: {experiment.experiment_state}')

    # Connect the user back to the experiment by re-adding the tap-X***vpn interface to the br-XM-X**** bridge 
    connect_user = False
    command = f'sudo python3 /home/aerpawops/AERPAW-Dev/DCS/platform_control/utils/ap-cf-ops-exp-user-connection.py {experiment.canonical_number.canonical_number} {connect_user}'
    """ aerpaw_thread = start_aerpaw_thread(request.user, experiment, AerpawThread.ThreadActions.SAVE_DEVELOPMENT)
    ssh_thread = threading.Thread(target=exp_user_connection,
                                args=(request, experiment, command, connect_user, MOCK_OPS, aerpaw_thread))
    ssh_thread.start() """

    print(f'Command: {command}')
    print(f'Connect User: {connect_user}')
    print('Sending Command')

    generate_user_messages_for_emulation(request, experiment=experiment)


def wait_emulation_schedule_to_active_development(request, experiment: AerpawExperiment):
    print()
    ...
    print(f'Ending current Emulation Session')
    end_scheduled_session(request, session, request.user)

    print(f' Creating and starting a new development session')
    dev_session = create_experiment_session('development', experiment, request.user)
    start_experiment_session(dev_session, request.user)
...
def wait_emulation_deploy_to_active_development(request, experiment: AerpawExperiment):
    session = ScheduledSession.objects.filter(
        experiment_id=experiment.id,
        session_type=ScheduledSession.SessionType.EMULATION,
        is_active=True
...
def active_emulation_to_active_development(request, experiment: AerpawExperiment):
    """ 
    - Reconnect the user to the experiment
    - End current Emulation session
    - Create a new Development session
    - Change experiment state from active emulation to active development
    - Set flags to 100 - emulation failed
    """
    print()
    print(f'active emulation to active development')
    # Get the current active emulation session
    session = ScheduledSession.objects.filter(
        experiment_id=experiment.id,
        session_type=ScheduledSession.SessionType.EMULATION,
        is_active=True
    ...
    # Change Experiment State to Active Deveoplment
    # Change Experiment Flags to 100 - Emulation Failed
    experiment.experiment_state = AerpawExperiment.ExperimentState.ACTIVE_DEVELOPMENT
...
def active_emulation_to_saving_emulation(request, experiment: AerpawExperiment):
    print('UTILS active_emulation_to_saved')
    """
    ACTIVE_EMULATION --> SAVED
    - emulation complete
    - Flags 100 or 101
    - Return the experiment to the save and exit state
    ...
    # UPDATE STATE: saved
    try:
        emulation_passed = request.data.get('is_success', False)
        exit_development = emulation_passed
    except Exception as exc:
        new_error(exc, request.user)
        emulation_passed = False
    if emulation_passed:
        experiment.experiment_state = AerpawExperiment.ExperimentState.SAVING_EMULATION
        experiment.save()
    ...
    try:
        aerpaw_thread = start_aerpaw_thread(request.user, experiment, AerpawThread.ThreadActions.SAVE_DEVELOPMENT)
        ssh_thread = threading.Thread(target=saving_emulation,
                                    args=(request, experiment, command, exit_development, mock, aerpaw_thread))
        ssh_thread.start()
...
def saving_emulation_to_active_emulation(request, experiment: AerpawExperiment):
    experiment.experiment_state = AerpawExperiment.ExperimentState.ACTIVE_EMULATION
    experiment.save()


def saving_emulation_to_saved(request, experiment: AerpawExperiment):
    session = ScheduledSession.objects.filter(
        experiment_id=experiment.id,
        session_type=ScheduledSession.SessionType.EMULATION,
        is_active=True
...
### Functions that send commands to the Control Framework ###
...
def saving_emulation(request, experiment: AerpawExperiment, command: str, exit_development: bool, mock: bool, aerpaw_thread: AerpawThread):
    exit_code = 0
    response = ''
    try:
        ssh_call = AerpawSsh(hostname=aerpaw_ops_host, username=aerpaw_ops_user, keyfile=aerpaw_ops_key_file, )
    ...
    if MOCK_OPS:
        ...
        time.sleep(5)
        exit_code = 0
        response = 'This is a MOCK of a successful response'
        print('response: ' + str(response))
        print('exit code: ' + str(exit_code))
    ...
    if exit_code == 0:
        # apcf_saveexit_ve_exp.py save-and-exit - success / failure
        # - saving_development --> saved
        saving_emulation_to_saved(request=request, experiment=experiment)
        end_aerpaw_thread(aerpaw_thread, exit_code, response)
    else:
        # - saving_development --> saved
        saving_emulation_to_active_emulation(request=request, experiment=experiment)
...
def exp_user_connection(request, experiment: AerpawExperiment, command: str, connect_user: bool, mock: bool, aerpaw_thread: AerpawThread):
    # Sends the command to connect/disconnect the user from their experiment for emulation        
    exit_code = 0
    response = ''
    try:
        ssh_call = AerpawSsh(hostname=aerpaw_ops_host, username=aerpaw_ops_user, keyfile=aerpaw_ops_key_file)
        ...
        print(f'Excpetion in exp_user_connection:\n {exc}\n')
        response = exc
        error = new_error(exc, request.user)
        add_error_to_thread(aerpaw_thread, error)
        exit_code = 1
    ...
    if MOCK_OPS:
        ...
        time.sleep(10)
        exit_code = 0
        response = "This is a MOCK successful response for experiment-user connection"
        print('response: ' + str(response))
        print('exit code: ' + str(exit_code))
    ...
    if exit_code == 0:
        if connect_user == True:
            active_emulation_to_active_development(request=request, experiment=experiment)
            end_aerpaw_thread(aerpaw_thread, exit_code, response)
    else:
...
def wait_development_deploy(request, experiment: AerpawExperiment, command: str, mock: bool, aerpaw_thread: AerpawThread) -> None:
    ...
    if MOCK_OPS:
        ...
        time.sleep(10)
        exit_code = 0
        response = "This is a MOCK successful response for initiating development"
        print('response: ' + str(response))
        print('exit code: ' + str(exit_code))
